...
@HiltViewModel
class NewContactorViewModel @Inject constructor(private val authService: AuthService,private val db: DataBaseService) :ViewModel(){

    private val user = ActualUser.getActualUser().name
    private val _uiState = MutableStateFlow(NewContactorUiState())
    val uiState : StateFlow<NewContactorUiState> = _uiState
    ...
    private fun getNewContactorId(): String {
        return _uiState.value.id.plus("_".plus(_uiState.value.refProduct)).uppercase()
    }

    @RequiresApi(Build.VERSION_CODES.O)
    fun addNewContactor(navigateToContactor: () -> Unit) {
        if(checkFormatNumber() && Connectivity.connectOk()==true){
            viewModelScope.launch {
                val deferred = withContext(Dispatchers.IO){
                    async {
                        db.getListOfProductByCollection(CONTACTORS).none { it.id==getNewContactorId() }
                    }
                }
                if (deferred.await()){
                    db.addNewProduct(kindOfCollection = CONTACTORS,
                        Product(id=getNewContactorId(), kind = _uiState.value.kind.uppercase(),
                    ...
                    _uiState.update {
                        it.copy(showDialogError = true, messageDialog = "REF Contactor ya existe")
                    }
                }
            }
        }else{
    ...
    private fun createRegisterNewContactor(): RegisterNewProduct {
        return  RegisterNewProduct(id=getNewContactorId(), refProduct = _uiState.value.refProduct.uppercase(),
...
data class NewContactorUiState(val id:String="CONTACTOR",
